...
const workerWrapperPath = require.resolve('./worker');
...
WorkerHandle.prototype._touch = function() {
  if (this._options.inactivityTimeout && this._options.inactivityTimeout !== Infinity) {
    ...
    }, this._options.inactivityTimeout);
  }
};
...
WorkerHandle.prototype._initWorker = function() {
  return this._send('init', { moduleName: this._options.workerModulePath }).then(function() {
    this._options.logger('info', this.toString(), ': Initialized');
...
WorkerHandle.prototype.discoverInterface = function() {
  var def = deferred();
  this._send('iface').then(function(reply) {
    def.resolve(reply.iface);
  });
  return def.promise;
};
 ...
 * @param callback
 */
WorkerHandle.prototype.invoke = function(method, args, callback) {
 ...
 *
 */
WorkerHandle.prototype.flush = function() {
  this._setState(State.flushing);
  return this._pipe.flush().then(function() {
    this._setState(State.processing);
  });
};
...
WorkerHandle.prototype._send = function(op, payload) {
  this._options.logger('debug', this.toString(), ': Sending ', payload);
  return this._pipe.send({
    op: op,
    payload: payload || {}
  });
};
