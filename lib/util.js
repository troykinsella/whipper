 ...
 * @return {*}
 */
function createLoadStrategyForName(nameOrFunction, pool, emitter, options) {
  var result;
  switch (nameOrFunction) {
    case 'basic':
      result = new (require('./load-strategy/' + nameOrFunction))(pool, emitter, options);
      break;
  }

  if (typeof nameOrFunction === 'function') {
    result = nameOrFunction(pool, emitter, options);
  }

  if (!result) {
    throw new Error("Invalid load strategy: " + nameOrFunction);
  }
  return result;
}
...
module.exports = {
  ...
  createLoadStrategyForName: createLoadStrategyForName,
